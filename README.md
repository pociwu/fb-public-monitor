...
- Chromium 金絲雀模式會沿用個人檔案頁面解析結果，或在 Apify 貼文不足／失敗時每個帳號最多 72 小時啟動一次補抓；每次最多處理 2 篇有固定連結的貼文，進入貼文及相片檢視器分批保存全部可存取照片。單篇每次最多新增 30 張、操作約 3 分鐘，每次切換隨機等待 3–7 秒；續抓位置與已見資產保存在 `browser-data/canary-album-progress.json`，下次從進度繼續。這些資料只補足內容，不參與貼文消失或刪除判定，也不另外抓留言。
