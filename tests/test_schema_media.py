...
@pytest.mark.asyncio
async def test_placeholder_profile_name_does_not_replace_known_name(tmp_path: Path):
    db = Database(tmp_path / "db.sqlite")
    db.execute(
        "INSERT INTO profiles(name,url,display_name,created_at,updated_at) VALUES(?,?,?,?,?)",
        ("FB-100", "https://facebook.com/100", "吳佳欣", "x", "x"),
    )
    ...
    await ingester.ingest(
        1,
        "profile",
        {"id": "100", "name": "FB-100", "url": "https://facebook.com/100"},
        notify=False,
    )

    assert db.row("SELECT display_name FROM profiles WHERE id=1")["display_name"] == "吳佳欣"
